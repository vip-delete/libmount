...
async function onMount(imgFile) {
  ...
  tmp?.open()?.writeData(codepage.encode("еПривет".repeat(1000)));
...
function showDir(info, root, fs, ctx) {
  ...
  const str = `Label: ${fs.getLabel()}, OEMName: ${fs.getOEMName()}, SerialNumber: 0x${fs.getId().toString(16).toUpperCase()}, SizeOfCluster: ${fs.getSizeOfCluster()}, CountOfClusters: ${fs.getCountOfClusters()}, FreeClusters: ${fs.getFreeClusters()}`;
      ...
      $("span").children([
        name.startsWith("/")
          ...
          : $("span")
              .text(name)
              .cls("name")
              .on("click", () => (f.isDirectory() ? onClick?.() : download(f?.open()?.readData() ?? null, name))),
      ]),
      $("span").text(f.getShortName() === name ? "" : f.getShortName()),
      $("span").text(f.length().toLocaleString()),
      $("span").text(f.getSizeOnDisk().toLocaleString()),
      $("span").text(formatDateTime(f.getCreationTime())),
      $("span").text(formatDateTime(f.getLastModified())),
      $("span").text(formatDate(f.getLastAccessTime())),
    ]).el;
}
